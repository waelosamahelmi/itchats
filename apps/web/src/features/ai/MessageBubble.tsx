...
import { Check, CheckCheck, Pause, Play, AlertCircle, RefreshCw, Loader2, Camera } from 'lucide-react';
...
export function MessageBubble({
  ...
  // ── Canvas loading placeholder for in-progress media generation ──
  if (message.kind === 'media_generating') {
    const isSelfie = message.text?.includes('selfie');
    const isVideo = message.text?.includes('video');
    const descMatch = message.text?.match(/"(.*?)"/);
    const mediaDesc = descMatch ? descMatch[1] : isSelfie ? 'a selfie' : isVideo ? 'a video' : 'an image';
    return (
      <article className="chat-message chat-message-character">
        ...
        <div className="message-stack">
          <div className="media-canvas-placeholder" role="status" aria-live="polite" aria-label={`Generating ${mediaDesc}`}>
            <div className="media-canvas-shimmer" />
            <div className="media-canvas-content">
              <div className="media-canvas-icon">
                <Camera size={28} strokeWidth={1.5} aria-hidden="true" />
              </div>
              <p className="media-canvas-label">
